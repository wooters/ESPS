...
gois_dist_td - compute the gain-optimized Itakura-Saito distortion
...
*gois_dist_td* computes the gain-optimized Itakura-Saito distortion
between two power spectra that are defined by *autocorrn* and
*codeword.* *gois_dist_td* computes the distortion by using equation (9)
in \[1\] in which *autocorrn* corresponds to the first spectral argument
and *codeword* corresponds to the second spectral argument. *autocorrn*
is a set of equally spaced, time-domain autocorrelations starting with
the zero lag autocorrelation, *autocorrn_err* is the linear predictive
analysis squared error that is associated with the *autocorrn* set of
autocorrelations, and *codeword* is the autocorrelation of
autoregressive filter coefficients. *size* is the number of *autocorrn*
and *codeword* values, and *gois_dist_td* returns the computed
distortion value.

# WARNINGS

*gois_dist_td* ignores the first term (element 0) in the arrays,
...
# EXAMPLE
     ...
     * the two power spectras, we would have to call gois_dist_td with the
     * following arguments:
     */
...
# SEE ALSO
    ...
    gnis_dist_td(3-ESPSsp), is_dist_td(3-ESPSsp)
