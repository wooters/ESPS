...
**hprecord** \[ **-s** *duration* \] \[ **-f** *sample rate* \] \[
...
*Hprecord* provides mono or stereo sampling at selected rates up to at
...
*stdout*. *Hprecord* has special adaptations that permit tight coupling
with *xwaves* (see *INTERACTION WITH XWAVES* below).

When no output file is specified, *hprecord* will, by default, write a
...
being first. Note that processes consuming the output of *hprecord* on a
...
This program is for HP Series 700 systems using the Audio Application
Program Interface (AAPI). To use this program, the audio server must be
enabled as detailed in the *Audio* (5) man page.

*hprecord* uses the standard environment variables used by the AAPI
library functions. As of this writing, these consist of **AUDIO** to set
the node of the audio server to use.
...
**-c** *channel***\[1\]**  
By default hprecord samples and stores single-channel data from the left
...
*Hprecord* is designed to optionally use the server mode of
...
following conditions must be met for this feature to work. (1)
*xwaves*(1-ESPS) must be running in the server mode. (2) The environment
variables WAVES_PORT and WAVES_HOST must be correctly defined (see
*espsenv*(1-ESPS)). (3) The record operation must be interrupted with a
SIGUSR1 signal or if *hprecord* is not thus interrupted, the -*S* flag
must have been set. (4) Output must be to a file.
...
(SIGUSR1). This causes *hprecord* to send the "make" command to
*xwaves*. If either kill -2 (SIGINT) or kill -3 (SIGQUIT) is sent to
*hprecord*, it will terminate gracefully, but will not send any messages
...
If the output of *hprecord* goes to the terminal, bad things will happen
...
*Hprecord* supports only the sampling rates supported by the hardware
that the AAPI communicates with. These cannot be guaranteed, but
typically they are (in Hz): 48000 44100 32000 22050 16000 11025 8000. If
you try to record a file at an unsupported rate, *hprecord* records the
...
# SEE ALSO
    ...
    hpplay(1-ESPS), sfconvert(1-ESPS),
