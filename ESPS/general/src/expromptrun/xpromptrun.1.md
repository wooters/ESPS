...
xpromprun - run ESPS program after interactive parameter entry in a
pop-up window

# SYNOPSIS

**xpromptrun** \[ **-P** *param_file* \] \[ **-n** \] \[ **-c**
*checkfile* \] \[ **-h** *help_name* \] \[ **-z** \] \[ **-x**
*debug_level* \] command_line

# DESCRIPTION

*Xpromptrun* is an X Windows program that displays an interactive form
for entering ESPS parameters, and then runs the corresponding ESPS
program. It is closely related to *xprompt* (1-ESPS), which just prompts
for parameters and produces a parameter file for subsequent use.
...
suitable **-P** option will be inserted by *xpromptrun*.

*Xpromptrun* processes the ESPS parameter file *param_file* and
...
Before displaying the interactive window, *xpromptrun* creates a
...
*xpromptrun* first creates the temporary parameter file, it fills it
...
*xpromptrun* window, the temporary parameter file is updated. Note that
...
*xpromptrun* window. The button DEFAULTS reverts all parameter values to
the defaults that were in the *param_file* (as updated by Common). The
button DONE causes *xpromptrun* to exit. Use this when you have finished
entering values. The state of the temporary output parameter file will
correspond to the state of the window when DONE is selected (assuming
that RETURNs have been typed as appropriate).
...
*xpromptrun* builds a new command line from *command_line* by inserting
a **-P** after the first item in *command_line* (which is assumed to be
a program name). The temporary parameter file created by *xpromptrun* is
used as the argument for the new **-P** option. The new command line is
then executed via *system*(3).

Note that *xpromptrun* does not terminate until after the command
generated from *command_line* has completed running. This makes
*xpromptrun* more useful in scripts and as a command tied to *xwaves*+
...
the file specified by the **-P** option). (Within *xpromptrun*, the
...
**-h** *help_name*  
Causes a HELP button to be provided in the *xpromptrun* window.
...
**-z**  
Specifies that *xpromptrun* operate silently; conditions such as "no
parameters" or "no indefinite parameters" are not reported.

**-x** *debug_level \[0\]*  
If *debug_level* is positive, *xpromptrun* outputs debugging messages.
...
No ESPS files are processed by *xpromptrun*.

# ESPS COMMON

Parameter processing by *xpromptrun* follows standard ESPS conventions:
Unless the parameter value is overridden by the value in ESPS Common,
the default value displayed by *xpromptrun* is the value from the
...
*xprompt* (1-ESPS), *getparam* (1-ESPS), *espsenv* (1-ESPS),
*read_params* (3-ESPS)
