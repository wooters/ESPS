...
v32play - send sampled data to a AT&T FAB2 DSP32, AT&T Surf DSP32C, or
Heurikon Surf DSP32C board

# SYNOPSIS

**v32play** \[ **-r** *range* \] \[ **-s** *start time* \] \[ **-e**
...
*V32play* sends all or a portion of one or more ESPS, SIGnal, NIST or
headerless sampled data files to a AT&T FAB2 DSP32, AT&T Surf DSP32C, or
a Heurikon Surf DSP32C board. The FAB2 board uses an on board codec
chip. The Surf board use an Ariel Pro-Port d/a converter. The FAB2 board
is single channel. The other boards are dual channel. A subrange of data
within the files may be chosen; this subrange may be specified in
seconds or sample points. Dual-channel (stereo) or single-channel
(monaural) data may be converted. Single-channel input data may be
directed to either or both output channels.
...
default is to play stereo if the file is stereo and to provide identical
output on both channels if the file is single-channel. If the file has a
header and is single channel, acceptable arguments to -*c* are 0 or 1 to
select channel A or B respectively for output. If the file has no
header, the default is to assume single-channel data and provide
identical output to both channels. For headerless files, acceptable
arguments to -*c* are 0 (output on channel A), 1 (output on channel B)
or 2 (stereo data, stereo output).

**-H**  
Force *v32play* to treat the input as a headerless file. This is
...
*V32play* is designed to optionally use the server mode of *xwaves*
(1-ESPS). This is especially handy when *v32play* is used as an *xwaves*
...
data view. *Xwaves* will send a signal 30 (SIGUSR1) to the play program.
V32play responds to this by sending back to *xwaves* a command "set
...
The SIGUSR1 signal to terminate *v32play* may come from any source. If
...
*V32play* may also be interrupted with kill -2 (SIGINT) or kill -3
(SIGQUIT). These signals are caught gracefully and *v32play* halts
...
*V32play* informs the user if the input file does not exist, if
...
*V32play* provides stereo D/A conversion at rates up to at least 48kHz
...
to *v32play* on a pipe must be able to keep up with the average
aggregate sampling frequency.

# FILES

**ESPS_BASE/32bin/** contains the DSP board code for the FAB2 board.

**ESPS_BASE/32cbin/** contains the DSP board code for the DSP32C board.
...
If a *v32play* is in progress when another board operation is requested
on the same machine, unpredictable results will occur.
...
simultaneous attempts to use the board.

Implement a robust check for loss of real-time operation.

# SEE ALSO

SD (5-ESPS), *testsd* (1-ESPS), *copysd* (1-ESPS), *v32record* (1-ESPS),
*sfconvert* (1-ESPS), *send_xwaves2* (3-ESPS)
